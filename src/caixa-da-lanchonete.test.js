...
describe("CaixaDaLanchonete", () => {
  ...
  it("Pedido vazio com dinheiro", () => {
    const pagamento = "dinheiro";

    const pedido = [];

    const user = new CaixaDaLanchonete();

    expect(user.calcularValorDaCompra(pagamento, pedido)).toBe(
      "Não há itens no carrinho de compra!"
    );
  });

  it("Pedido de um sanduiche e um chantily com dinheiro", () => {
    const pagamento = "dinheiro";
  ...
  it("Pedido de um sanduiche, um café e um chantily com dinheiro", () => {
    const pagamento = "dinheiro";

    const pedido = [
        {produto: 'cafe', quantidade: 1},
        { produto: "chantily", extra: "cafe", quantidade: 1 },
        { produto: "sanduiche", quantidade: 1 },
    ];
  ...
  it("Pedido que não está no cardápio", () => {
    const pagamento = "dinheiro";
    ...
    const pedido = [
        ...
        { produto: "chantily", extra: "cafe", quantidade: 1 },
        { produto: "sanduiche", quantidade: 1 },
        { produto: "torta", quantidade: 1 },
    ];

    const user = new CaixaDaLanchonete();

    expect(user.calcularValorDaCompra(pagamento, pedido)).toBe(
      "Esse produto não está no cardápio"
    );
  });

  it("Pedido pago com cartão de crédito", () => {
    const pagamento = "credito";
